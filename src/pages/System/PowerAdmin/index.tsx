...
// 类型声明
// ==================
import { TableRecordData, ModalType, operateType, Menu, Power, PowerParam, Res } from './index.type';
...
function PowerAdminContainer(props: Props) {
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      key: 'conditions',
      render: (v: number) => (v === 1 ? <span style={{ color: 'green' }}>启用</span> : <span style={{ color: 'red' }}>禁用</span>),
    },
    {
      title: '操作',
      key: 'control',
    ...
    <div className="page-power-admin">
      ...
      <div className="r">
        <div className="searchBox">
          <ul>
            <li>
              <Button type="primary" icon={<PlusCircleOutlined />} onClick={() => onModalShow(null, 'add')} disabled={!(treeSelect.id && p.includes('power:add'))}>
      ...
      {/** 查看&新增&修改用户模态框 **/}
      <Modal
        title={`${{ add: '新增', up: '修改', see: '查看' }[modal.operateType]}权限: ${treeSelect.title}->${modal.nowData?.title ?? ''}`}
      ...
      >
        <Form form={form} initialValues={{ formConditions: 1 }}>
          ...
          <Form.Item label="赋予" {...formItemLayout}>
            <Checkbox.Group disabled={modal.operateType === 'see'} options={rolesCheckboxData} value={rolesCheckboxChose} onChange={(v: CheckboxValueType[]) => setRolesCheckboxChose(v as number[])} />
          </Form.Item>
        </Form>
      </Modal>
    </div>
