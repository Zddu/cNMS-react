...
import { Page, TableRecordData, operateType, ModalType, PowerTreeInfo, SearchInfo, RoleParam, Role, Res, Props } from './index.type';
...
function RoleAdminContainer(props: Props) {
      ...
      if (modal.operateType === 'add') {
        ...
        try {
          ...
          if (res && res.status === 200) {
            ...
            getData(page);
            dispatch.app.updateUserInfo(store.getState()); // 角色信息有变化，立即更新当前用户信息
            onClose();
          }
        } finally {
          setModal({
        ...
        try {
          ...
          if (res && res.status === 200) {
            ...
            getData(page);
            dispatch.app.updateUserInfo(store.getState());
            onClose();
          }
        } finally {
          setModal({
  ...
  const onAllotPowerClick = (record: TableRecordData) => {
    ...
    // 需默认选中的权限ID
    const powers = record.menuAndPowers.reduce((v1, v2) => [...v1, ...v2.powers], []);
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      key: 'conditions',
      render: (v: number, record: TableRecordData) => (v === 1 ? <span style={{ color: 'green' }}>启用</span> : <span style={{ color: 'red' }}>禁用</span>),
    },
    {
      title: '操作',
      key: 'control',
    ...
    <div>
      ...
      <Modal title={{ add: '新增', up: '修改', see: '查看' }[modal.operateType]} visible={modal.modalShow} onOk={() => onOk()} onCancel={() => onClose()} confirmLoading={modal.modalLoading}>
        ...
        >
          ...
          >
            <Input placeholder="请输入角色名" disabled={modal.operateType === 'see'} />
          </Form.Item>
