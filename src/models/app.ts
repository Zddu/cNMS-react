...
import { Menu, Role, Power, MenuAndPower, UserInfo, AppState, Res } from './index.type';
...
export default {
  ...
  effects: (dispatch: Dispatch) => ({
    ...
    async updateUserInfo(rootState: RootState): Promise<any> {
      ...
      const roles: Role[] = res2.data.filter((item: Role) => item.conditions === 1);

      /** 3.根据菜单id 获取菜单信息 **/
      const menuAndPowers = roles.reduce((a, b) => [...a, ...b.menuAndPowers], [] as MenuAndPower[]);
      ...
      const menus: Menu[] = res3.data.filter((item: Menu) => item.conditions === 1);

      /** 4.根据权限id，获取权限信息 **/
      const res4: Res | undefined = await dispatch.sys.getPowerById({
        id: Array.from(new Set(menuAndPowers.reduce((a, b) => [...a, ...b.powers], [] as number[]))),
      });
      if (!res4 || res4.status !== 200) {
        // 权限查询失败
        return res4;
      }
      const powers: Power[] = res4.data.filter((item: Power) => item.conditions === 1);
      this.setUserInfo({
        ...userinfo,
        roles,
        menus,
